          ...
          <div className="mt-6 space-y-5">
            ...
            <FormSection icon={Compass} title={tr('Pace and interests', 'Nhịp độ và sở thích')}>
              <RadioGroup value={pace} onValueChange={(value) => setPace(value as PaceId)} aria-label={tr('Trip pace', 'Nhịp độ chuyến đi')} className="grid gap-2 sm:grid-cols-3">
                {PACES.map((item) => <Radio key={item.id} value={item.id} className={cn('w-full justify-start rounded-xl border px-3 py-3 text-left sm:flex-col sm:justify-center sm:px-2 sm:text-center', pace === item.id ? 'border-brand bg-accent' : 'border-border bg-card hover:bg-tint')}><span className="text-xs font-bold text-foreground">{tr(item.label, item.labelVi)}</span><span className="mt-1 text-3xs leading-snug text-body">{tr(item.detail, item.detailVi)}</span></Radio>)}
              </RadioGroup>
