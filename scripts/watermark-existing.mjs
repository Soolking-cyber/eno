...
// ⛔ "KEEP IN SYNC" HAS FAILED THREE TIMES IN THIS FILE — the wordmark, the shadow, and the mark
// SIZE (15% here vs 28% in the app, found 2026-09-10 when the owner asked why watermarks looked
// inconsistent). A comment is not a mechanism. If this script is ever run again, port it to .ts and
// import watermarkPlacement/watermarkSvg/inkForLuminance from src/lib/core/watermark-mark.ts —
// they are pure and export-ready precisely so this copy can stop existing.
...
const MARK_H_RATIO_W = MARK_W, MARK_H_RATIO_H = MARK_H
/**
...
async function main() {
  ...
  for (const url of urls) {
    ...
    try {
      ...
      // ⛔ THIS LINE HAD DRIFTED AND WAS THE FILE'S THIRD DRIFT, AFTER THE WORDMARK AND THE SHADOW.
      // It read `Math.min(300, Math.max(72, round(info.width * 0.15)))` — 15% of the width capped
      // at 300 — while src/lib/core/watermark-mark.ts used 28% floored at 190, so an image this
      // script marked carried a wordmark roughly HALF the size of one the app marked. Same header,
      // same claim to mirror, ~1.9× apart on the page.
      // ⚠️ THE PADDING WAS DRIFTING TOO: `width * 0.025` here against `min(width, height) * 0.03`
      // in the app — 40px vs 27px on a 1600×900 photo. Matching the mark size alone would still
      // have put it somewhere else. Both now mirror watermarkPlacement() exactly.
      const pad = Math.round(Math.min(info.width, info.height) * 0.03)
      const wanted = Math.min(580, Math.max(120, Math.round(info.width * 0.28)))
      const byHeight = Math.floor(((info.height - 2 * pad) * MARK_H_RATIO_W) / MARK_H_RATIO_H)
      const mw = Math.max(1, Math.min(wanted, info.width - 2 * pad, byHeight))
