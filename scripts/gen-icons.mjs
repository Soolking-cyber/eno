...
import { CRITICAL_GLYPHS } from './critical-icons.mjs'
 ...
 */
const symbolsFor = (glyphs) => glyphs
  .map((g) =>
...
/**
 * ⛔ TWO SPRITES, AND THE SPLIT IS THE WHOLE POINT — ONE FILE WAS A REGRESSION I SHIPPED.
 *
 * The first cut of this put all 486 symbols in one 185 KB file. An external `<use>` target is
 * fetched at Chrome's `Low` priority, so on the 1.6 Mbps / 4x-CPU profile this audience actually
 * has, that file did not finish until 5.9–7.3 s — and at 2.2 s there were 310 `<use>` elements
 * with a ZERO-SIZE BOX. The bottom nav rendered as five bare text labels and cards had no heart or
 * location glyph, while the text and photographs were fully painted. Measured, not theorised.
 *
 * The four highest-traffic routes reference 39 of the 243 glyphs on first paint
 * (scripts/critical-icons.mjs, and that list is measured from the DOM, not curated). So the core
 * file carries those and the rest defer. Measured saving: ~155 KB off every page load, home `load`
 * 8,138 → 5,722 ms.
 *
 * ⚠️ EACH COMPONENT CARRIES ITS OWN SPRITE URL, so a MIS-PARTITIONED GLYPH COSTS ONE EXTRA REQUEST
 * AND NEVER A BLANK ICON. There is no lookup to miss and no fallback to fail. That property is what
 * makes it safe for the critical list to be slightly wrong, and it must survive any refactor here —
 * do not collapse it into a shared default sprite.
 */
const CRITICAL = new Set(CRITICAL_GLYPHS)
for (const name of CRITICAL) {
  if (!spriteGlyphs.some((g) => g.name === name)) {
    fail(`critical-icons.mjs lists "${name}", which is not a glyph this generator emits`)
  }
}
...
const wrap = (glyphs, hash) =>
  ...
  symbolsFor(glyphs) + `</svg>`

const hashOf = (glyphs) => createHash('sha256').update(symbolsFor(glyphs)).digest('hex').slice(0, 12)
const coreHash = hashOf(coreGlyphs)
const restHash = hashOf(restGlyphs)
 ...
 * ⚠️ The two files hash INDEPENDENTLY: editing a deferred glyph must not bust the core file that
 * every page preloads.
 */
const coreUrl = `/icons/glyphs-core.svg?v=${coreHash}`
const restUrl = `/icons/glyphs-rest.svg?v=${restHash}`

// Sweep CONTENT-HASHED sprites left by an older revision of this script. ⚠️ Deliberately NOT
// `glyphs.svg` — see the transitional write below.
for (const f of readdirSync(OUT)) {
  if (/^glyphs\.[0-9a-f]+\.svg$/.test(f)) rmSync(join(OUT, f))
}
writeFileSync(join(OUT, 'glyphs-core.svg'), wrap(coreGlyphs, coreHash))
writeFileSync(join(OUT, 'glyphs-rest.svg'), wrap(restGlyphs, restHash))

/**
 * ⛔⛔ THE TRANSITIONAL FULL SPRITE — DO NOT DELETE THIS UNTIL THE HTML CACHE HAS TURNED OVER.
 *
 * The single-file sprite shipped at `/icons/glyphs.svg?v=<hash>`, and eno.vn edge-caches its HTML
 * for six hours (`s-maxage=21600`). So for six hours after this deploy, real visitors are served
 * HTML that references THAT path. Splitting the file without leaving it behind removes it from the
 * image, every `<use>` 404s, and a `<use>` whose target 404s draws NOTHING — blank icons across the
 * entire site, with a green build and no error anywhere.
 *
 * ⚠️ I WROTE THE COMMENT WARNING ABOUT THIS AND THEN DID IT ANYWAY. The rule at the head of the URL
 * block — "a stable name cannot 404" — was written for exactly this hazard, and the first cut of
 * the split changed the name regardless; the sweep regex above even deleted the old file. Two
 * independent reviewers caught it. This write is the fix: old HTML keeps resolving, new HTML uses
 * the split pair.
 *
 * ⚠️ REMOVING IT IS A SEPARATE, LATER COMMIT. Safe once no cached HTML references it — i.e. at
 * least 6 hours after the deploy that ships the split, and after a Cloudflare `purge_everything`.
 * Until then it costs 185 KB of disk in the image and ZERO bytes to any visitor, because nothing
 * in the current build points at it.
 */
writeFileSync(join(OUT, 'glyphs.svg'), wrap(spriteGlyphs, hashOf(spriteGlyphs)))

const shimBody = spriteGlyphs
  .map((g) => `/** ${g.why} */\nexport const ${g.name} = (p: IconProps) => ` +
    `<Glyph {...p} n="${g.name}" s={${CRITICAL.has(g.name) ? 'C' : 'R'}} />`)
  .join('\n')
 ...
 *
 * ⛔ \`ICON_SPRITE_CORE\` CARRIES THE 39 GLYPHS THE FOUR BUSIEST ROUTES PAINT ON ARRIVAL, and it is
 * the ONLY one \`src/app/layout.tsx\` preloads. The deferred file is fetched lazily by the browser
 * when a \`<use>\` in it is first rendered — which for most pages is never.
 * ⚠️ Do NOT preload the deferred file "to be safe": that reinstates exactly the 185 KB on the
 * critical path this split exists to remove.
 */
export const ICON_SPRITE_CORE = '${coreUrl}'
export const ICON_SPRITE_REST = '${restUrl}'

/** Back-compat alias — the core file is what any single-sprite caller wants. */
export const ICON_SPRITE_URL = ICON_SPRITE_CORE

const C = ICON_SPRITE_CORE
const R = ICON_SPRITE_REST
 ...
 *
 * ⚠️ \`s\` IS THE GLYPH'S OWN SPRITE URL, PASSED PER COMPONENT RATHER THAN READ FROM A SHARED
 * CONSTANT. That is what makes the critical/deferred partition fail safe: a glyph placed in the
 * wrong file still resolves, it just arrives with the other request. There is no lookup to miss.
 *
 ...
 */
function Glyph({ size = 24, strokeWidth: _sw, absoluteStrokeWidth: _asw, n, s, ...rest }: IconProps & { n: string; s: string }) {
  return (
    <svg
      ...
      {...rest}
    >
      <use className="i-rest" href={\`\${s}#\${n}-r\`} />
      <use className="i-on" href={\`\${s}#\${n}-o\`} />
    </svg>
  )
}
