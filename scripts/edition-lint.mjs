...
const ALLOW = new Map([
  ...
  ["src/app/api/seller/payout/route.svc.ts", "Both reads are `db.seller.findUnique({ where: { ownerId: userId } })` from route({auth:'userId'}) \u2014 no target id is ever accepted from the client. It selects only `id`, and that id is used solely as the key of a seller_payout upsert/read; no Listing is touched and no seller field reaches the response (the GET returns a boolean, a bank BIN and a masked last-four). It is also `.svc.`, so the file does not compile into the marketplace build at all."],
