...
const ALLOW = new Map([
  ...
  ["src/lib/core/storage-purge.ts", "The account-deletion reference check. Every count MUST see EVERY row — desk/visa listings included — because a match means 'another row still uses this object, do not delete it'; scoping the read to the marketplace would make an object a hidden listing uses look unreferenced and delete it. Runs only after the departing user's own rows are gone, on canonical first-party URLs, and never returns row data."],
  ["src/lib/core/account-erasure.ts", "The ONE account-erasure procedure (moved out of api/account/delete on 2026-09-05 so the admin Users console can share it). Seller/listing reads are `where: { ownerId: profile.id }` / `where: { sellerId: s.id }` for the account being erased — the caller resolved the profile from the session (self-service) or from an `auth: 'admin'` route; no read here is a listing feed, and every row it touches is deleted in the same transaction."],
  ["src/lib/admin-users.ts", "The admin Users console (route({auth:'admin'}) on every caller). Seller reads are nested selects on the Profile the operator opened or searched for; the one listing read is `count({ sellerId })` for that account's own storefront. Nothing here is a public feed and nothing returns listing rows — an operator who can see the desk seller's listing COUNT is the point of an admin console."],
  ...
  ["src/app/api/cron/affiliate-prices/route.ts", "CRON_SECRET bearer via route({auth:'cron'}); every read is pinned to ONE named merchant storefront (seller.name === merchantNameFor(campaign), ownerId untouched by this route) and to rows carrying an externalId \u2014 i.e. only datafeed-imported affiliate products, which the desk seller has none of. Selects id/externalId/price/affiliateUrl; the response is counts plus revalidatePath calls, so no listing field is ever serialized to a caller."],
  ...
  ["src/app/api/seller/payout/route.forum.svc.ts", "Both seller reads are keyed `where: { ownerId: userId }` (and `where: { id: sellerId }` where `sellerId` was itself resolved from `ownerId: userId`) under route({auth:'userId'}) \u2014 no target id is ever accepted from the client, so a seller can only ever read their OWN storefront. The GET returns whether payouts are configured, a bank BIN, a masked last-four, and a `suggestedName` derived from the caller's own `legalName`/`taxRegisteredName` (business) or verified identity (individual) \u2014 all the seller's own data, never another seller's, and no Listing is touched. It is `.forum.svc.`, so it does not compile into the marketplace build at all."],
