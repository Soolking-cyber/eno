...
// ⛔ PANEL SINCE 2026-09-14 (afternoon) = astra + agy + opus, all on high. Owner, that morning: "2nd opinions update agy
// no token codex use astra in high and opus 5 in high" — agy was out of subscription quota (429 "Individual quota
// reached") and the gpt-5.6-sol seat was dropped for astra. Hours later, once the quota reset: "add agy back to 2nd
...
const UNREVIEWABLE = [
  ...
  /**
   * ⛔ GOOGLE'S PUBLISHED PRODUCT TAXONOMY, VERBATIM — 5,595 third-party lines nobody here writes
   * or edits, regenerated by the `curl` in its own header. Measured 2026-09-20: it is 490KB of a
   * 518KB staged diff whose actual CODE change is 28KB, and at that size agy is truncated, stops
   * counting, and the two-seat panel drops to one lab — so the gate could not read a 28KB change
   * because of the data file sitting beside it. That is the exact failure this list exists for.
   * ⚠️ IT IS STILL HASHED. Altering one line of it still moves the receipt hash and still
   * invalidates the review; only the text SENT to the reviewers is trimmed. A reviewer cannot
   * meaningfully audit Google's taxonomy anyway — the thing worth reviewing is which ids the MAP
...
// the wire and still hit E2BIG. The cut is made on the byte buffer; a multi-byte character split at the edge decodes
// as one replacement mark, which is harmless in a prompt that already says it was truncated.
const promptBytes = Buffer.from(prompt, 'utf8')
const agyTruncated = promptBytes.length > AGY_LIMIT
