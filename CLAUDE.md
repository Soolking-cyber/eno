...
| **Second opinion** (plan AND finished diff; ALSO every security/bug audit — owner 2026-07-23) | `antigravity` (agy) + Opus 5.5 · high | ⏸ **Since 2026-09-25: agy `gemini-3.8-flash-high` + Opus 5.5 (`claude-opus-5-5`, effort high).** Owner: *"drop codex till available"* (OpenAI quota exhausted) and *"make sure the 2n opinion is opus 5.5 with high reasoning"*. agy is the only independent lab, so say "single-sourced". To bring codex back (owner's word only): `codex exec -m gpt-5.6-sol -c model_reasoning_effort=medium -c web_search=disabled --skip-git-repo-check --sandbox read-only < prompt.txt`. |
...
| Commit-gate panel | `scripts/second-opinion.mjs` | ⏸ **agy `gemini-3.8-flash-high` + Opus 5.5 (`claude-opus-5-5`) · high, since 2026-09-25; codex paused.** Two seats, two labs; the gate needs both. Past agy's 180KB cutoff agy does not count, so the gate REFUSES — split the change. Opus writes most diffs here, so a REFUTED from agy is the real objection — go and measure it. |
...
- ⏸ **codex PAUSED 2026-09-25 — owner: *"drop codex till available"*.** Its OpenAI quota ran out and build agents slept waiting for it. The panel is **agy + Opus 5.5 (`claude-opus-5-5`, effort high — owner 2026-09-25)** until the owner says codex is back (restore = `REVIEWER_NAMES` AND the commented seat in `scripts/second-opinion.mjs`, both). ⛔ Never sleep waiting for a reviewer's quota reset. The gate still needs BOTH agy and Opus to answer (two labs): if agy fails, retry it; past agy's 180KB cutoff the gate refuses — split the change. agy is the only independent seat while codex is paused, so the review is single-sourced by construction.
- (HISTORY, paused 2026-09-25) ⛔ **SINCE 2026-09-24 THE PANEL IS agy + codex (`gpt-5.6-sol`, medium) + Opus 5** — owner re-added the OpenAI seat; the two bullets below describe the 2026-09-23 agy+Opus state and are HISTORY.
...
- ⛔ **PAGES LIVE UNDER `src/app/[lang]/` (2026-09-17) — the HTML is server-rendered in the visitor's language.** `src/proxy.ts` rewrites every page request to `/en/…` or `/vi/…` (cookie `lang`, else Accept-Language; rule in `src/lib/lang-variant.ts`); public URLs never change and a public `/en/…` or `/vi/…` 404s. Consequences: **purge with `revalidatePublicPath()`** (`src/lib/revalidate-lang.ts`) — a bare `revalidatePath('/listings/x')` purges nothing; `next.config.ts` page-path rewrite sources need `/:lang(en|vi)`; ESLint globs must escape `\\[lang\\]`; and the Cloudflare HTML edge-cache rule must not cache page HTML (Free plan cannot key the cache on language). Only en/vi render on the server; the nine MT languages get the English variant and swap client-side.
