...
1. **Reproduce first, or say you can't.** A fix for a bug you never observed is a guess. Drive the actual flow: `npm run build` + the standalone server via `node scripts/preview.mjs vn` (:3000 — it force-claims the port and prints `── serving` only once it holds it; never poll for a 200, that cannot tell you WHOSE server answered), a targeted Playwright spec, a `node` script against `DIRECT_URL`, whatever exercises it.
