...
| 4 | **Google sign-in is dead** — the self-hosted GoTrue has no Google provider configured, and Google's OAuth client does not know `sb.eno.vn/auth/v1/callback` | ⚠️ **BUILT AND TESTED, blocked on one credential.** Our own button and our own OAuth round-trip already exist (14/14 tests pass) so the consent screen reads *eno.vn*; `GOOGLE_CLIENT_SECRET` exists nowhere and only the Google Console has it. See `google-signin.md`, then run `apply-google-signin.sh` |
