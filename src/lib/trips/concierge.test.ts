...
describe('⚠️ the shared-desk gate — SOURCE-LEVEL, because losing it is a cross-desk bug', () => {
  ...
  it.each([
    ...
    ['app/api/trips/help/route.forum.svc.ts'],
...
describe('⚠️ the person/AI toggle must work in BOTH directions', () => {
  ...
  it('⚠️ does NOT pretend the trips read-then-write is atomic', () => {
    ...
    const route = read2('app/api/trips/help/route.forum.svc.ts')
...
describe('⚠️ a rejected generate must never be a dead end, whatever the cause', () => {
  ...
  it('the route records WHICH field failed, never its value', () => {
    const route = readSrc('app/api/itineraries/generate/route.forum.svc.ts')
