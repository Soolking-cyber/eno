  ...
  children.push(
    new Table({
      ...
      rows: [new TableRow({ children: [new TableCell({
        ...
        borders: { top: noBorder, bottom: noBorder, left: noBorder, right: noBorder },
        children: [
          new Paragraph({ spacing: { after: 150 }, children: [docRun({ text: 'eno', bold: true, color: WHITE, size: 24 }), docRun({ text: `.forum  /  ${tr('Vietnam itinerary', 'Lịch trình Việt Nam').toUpperCase()}`, bold: true, color: 'B8D9F7', size: 17, characterSpacing: 30 })] }),
  ...
  const document = new Document({
    ...
    sections: [{
      ...
      headers: { default: new Header({ children: [new Paragraph({
        ...
        children: [docRun({ text: 'eno', bold: true, color: BRAND, size: 19 }), docRun({ text: '.forum  •  ', bold: true, color: INK, size: 17 }), docRun({ text: plan.routeSummary, color: MUTED, size: 16 })],
      })] }) },
      footers: { default: new Footer({ children: [new Paragraph({
        alignment: AlignmentType.CENTER,
        children: [docRun({ children: ['www.eno.forum  •  ', PageNumber.CURRENT], color: MUTED, size: 15 })],
      })] }) },
      children,
    }],
  })
