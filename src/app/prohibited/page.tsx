...
const GROUPS: { id: string; title: string; items: string[] }[] = [
  ...
  },
  {
    ...
    items: [
      'Medicines of every kind — prescription, over-the-counter and herbal (online sale is restricted to licensed pharmacies with ordering functions, which eno.vn does not provide)',
...
export default function ProhibitedPage() {
  return (
    <ContentPage
      ...
      meta={<p className="mt-2 text-xs text-ink-4"><Tr text="Part of the Operating Regulations · Last updated: July 2026" /></p>}
    ...
    >
      ...
      <ContentSection id="enforcement" title="Enforcement">
        <p className="text-sm leading-relaxed text-body"><Tr text="Every new listing passes an automated screen (banned-keyword filter, photo requirement, contact-in-text scan) before going live, and every listing can be reported by any user. Confirmed violations are removed, penalize the seller's trust score per the published penalty table, and repeat or severe violations escalate through restriction to a permanent ban. Content identified by a competent state authority is removed within 24 hours of the request. If you spot something that should not be here, use the Report button — it genuinely helps." /></p>
      </ContentSection>
    </ContentPage>
  )
}
