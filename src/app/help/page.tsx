  ...
  return (
    <div className="flex min-h-screen flex-col blob-bg">
      ...
      <main className="flex-1 max-w-3xl mx-auto w-full px-4 sm:px-6 lg:px-8 pt-10 pb-16">
        <p className="eyebrow text-[#0a66c2] mb-2">Help center</p>
        <h1 className="h-display text-[#1a202c]">Frequently asked questions</h1>
        <div className="mt-8 divide-y divide-slate-200">
          {faqs.map(([q, a], i) => (
            <div key={i} className="py-5">
              <h3 className="text-base font-bold text-[#1a202c]">{q}</h3>
              <p className="mt-1.5 text-sm leading-relaxed text-[#475569]">{a}</p>
            </div>
          ))}
        </div>
        <p className="mt-8 text-sm text-[#475569]">
          Still stuck? Email <a href="mailto:support@eno.forum" className="font-semibold text-[#0a66c2] hover:underline">support@eno.forum</a>.
        </p>
      </main>
      <Footer />
    </div>
