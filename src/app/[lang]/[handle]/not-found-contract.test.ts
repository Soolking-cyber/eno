 ...
 *
 * `src/app/[lang]/[handle]` is the root dynamic segment, so it doubles as the catch-all for every unknown
 ...
 *   GET /nope/xyz/abc     404, 98,031 bytes, **56 anchors** — src/app/[lang]/not-found.tsx, fully rendered.
...
const HANDLE_DIR = join(process.cwd(), 'src/app/[lang]/[handle]')
...
describe('the /_not-found document keeps its agent recovery links', () => {
  const notFound = readFileSync(join(process.cwd(), 'src/app/[lang]/not-found.tsx'), 'utf8')
 ...
 *   'src/app/[lang]/[handle]/not-found-contract.test.ts'
 *
