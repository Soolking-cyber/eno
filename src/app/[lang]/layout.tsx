...
/**
 * ⛔ THE ROOT LAYOUT LIVES UNDER `[lang]` SO THE FIRST HTML IS IN THE VISITOR'S LANGUAGE.
 * src/proxy.ts rewrites every page request to `/en/…` or `/vi/…` (public URLs unchanged); this segment
 * turns that into `<html lang>` and the provider's starting language, and ISR keeps one copy per
 * variant. Only `en` and `vi` exist — the proxy never emits anything else, and a public `/en/…` or
 * `/vi/…` 404s there — so anything else reaching here is a bug and 404s too.
 */
export function generateStaticParams() {
  return LANG_VARIANTS.map((lang) => ({ lang }));
}
...
export default async function RootLayout({
  ...
  if (!(LANG_VARIANTS as readonly string[]).includes(raw)) notFound();
  const lang = raw as LangVariant;
  return (
