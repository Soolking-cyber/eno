...
export async function approveIdentityAction(verificationId: string, nationality?: string): Promise<ReviewResult> {
  ...
  if (result.ok) revalidatePublicPath('/admin/verification')
  return result
}
...
export async function correctIdentityAction(input: {
   ...
   * `revalidatePublicPath('/admin/users')` revalidates the LIST path only — the detail page it was meant
   ...
   */
  if (result.ok) revalidatePublicPath('/admin/users/[id]', 'page')
  return result
}
