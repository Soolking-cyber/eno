...
// ⚠️ The limits and the counter key are NOT local any more. Accommodation refining shares this
// trip's twelve, and it only genuinely shares them if neither route owns the key — see that file.
...
// ⚠️ WS6 — NOT MIGRATED: the same three wire facts as the sibling `stays/suggest` route.
...
// · The body is read BOUNDED (4 kB, checked on `content-length` AND on the delivered text) and an
//   oversized one is answered `{"error":"body_too_large"}` at 413. route()'s parser calls
//   `req.json()` with no ceiling and answers one 400 code, so `body:` deletes the 413 branch.
// · There is no route-level limiter to hoist. The meter is `aiGuard('itinerary-refine', …)` plus
//   `spendRefinement(itineraryId)` — a lifetime counter keyed on the ITINERARY, not on the caller,
//   which `rateLimit:` cannot express — and both must run AFTER the ownership 404s below so a
//   stranger's `dayId` costs no quota.
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  // ⚠️ ONE TRIP, ONE TALLY — shared with accommodation refining, which is why the increment, the cap
  // and the refund-the-refusal live in `@/lib/itinerary-refine` rather than here. It still
  // INCREMENTS THEN CHECKS (two tabs reading "11 used" would both see room and both spend), and it
  // still fails CLOSED when the counter is unreachable, unlike the generation slot: that guard
  // protects spend efficiency and its failure costs a duplicate, while this one IS the cap.
