            ...
            {item.reports.map((r) => (
              <div key={r.id} className="space-y-1.5">
                ...
                <div className="flex flex-wrap gap-1.5 pl-4">
                  ...
                  <button onClick={() => run('dismiss-report', r.id)} disabled={!!busy} className="rounded-md border border-slate-300 bg-white px-2 py-0.5 text-[10px] font-bold text-[#1a202c] hover:bg-slate-50 disabled:opacity-40 cursor-pointer">Dismiss</button>
                  <button onClick={() => run('abusive-report', r.id)} disabled={!!busy} className="rounded-md border border-amber-300 bg-white px-2 py-0.5 text-[10px] font-bold text-amber-700 hover:bg-amber-50 disabled:opacity-40 cursor-pointer">Abusive (penalize reporter)</button>
                </div>
              </div>
            ))}
          </div>
...
function AccountReportRow({ r }: { r: AccountReport }) {
  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-white p-4">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <p className="truncate text-sm font-bold text-[#1a202c]">{r.targetName || r.targetSellerId || 'Unknown storefront'}</p>
          <div className="mt-1 flex items-start gap-1.5 text-[11px] text-red-800">
            <Flag className="mt-0.5 h-3 w-3 shrink-0" />
            <span><strong>{REASON_LABEL[r.reason] || r.reason}</strong>{r.detail ? ` — ${r.detail}` : ''}</span>
          </div>
        </div>
        <span className="shrink-0 text-[11px] text-[#94a3b8]">{new Date(r.createdAt).toLocaleDateString('en-GB', { day: 'numeric', month: 'short' })}</span>
      </div>
      <div className="mt-3 flex flex-wrap items-center gap-1.5">
        ...
        <button onClick={() => run('dismiss-report')} disabled={busy} className="rounded-md border border-slate-300 bg-white px-2 py-0.5 text-[10px] font-bold text-[#1a202c] hover:bg-slate-50 disabled:opacity-40 cursor-pointer">Dismiss</button>
        <button onClick={() => run('abusive-report')} disabled={busy} className="rounded-md border border-amber-300 bg-white px-2 py-0.5 text-[10px] font-bold text-amber-700 hover:bg-amber-50 disabled:opacity-40 cursor-pointer">Abusive (penalize reporter)</button>
        {busy && <Loader2 className="h-3.5 w-3.5 animate-spin text-[#64748b]" />}
        {error && <span className="text-xs font-semibold text-red-600">{error}</span>}
      </div>
    </div>
  )
    ...
    <div>
      ...
      {tab === 'accounts' ? (
        accountReports.length === 0 ? (
          <div className="rounded-2xl border border-dashed border-slate-200 py-16 text-center text-sm text-[#94a3b8]">No open account reports.</div>
        ) : (
