...
describe('badge plates — ink clears AA over its own wash on the worst surface', () => {
  ...
  it('the plate rule and both call sites still use these tokens', () => {
    ...
    // ⛔ Tailwind's build emits a SOLID `var(--plate-tint)` for any color-mix() on a var(), unconditionally — measured in
    // two deploys. Only the not-supported block clears it on Safari 16.0–16.1, where ink on its own colour is invisible.
    expect(css).toMatch(/@supports not \(background-color: color-mix\([^)]*\)\)\s*\{\s*\.badge-plate\s*\{[^}]*background-color:\s*transparent/)
