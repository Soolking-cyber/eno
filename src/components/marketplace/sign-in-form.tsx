 ...
 *  handlers (Google/Facebook OAuth, email magic-link, phone OTP). */
    ...
    <div className={cn('space-y-3', className)}>
      ...
      {tab === 'phone' && stage === 'code' && (
        <div className="space-y-3">
          ...
          <InputOTP maxLength={6} value={code} onChange={setCode} onComplete={onCodeComplete} autoFocus containerClassName="justify-center" disabled={loading}>
            <InputOTPGroup>
              {[0, 1, 2, 3, 4, 5].map((i) => (
                <InputOTPSlot key={i} index={i} className="h-12 w-12 text-lg font-semibold data-[active=true]:border-[#0a66c2] data-[active=true]:ring-[#0a66c2]/30" />
              ))}
            </InputOTPGroup>
          </InputOTP>
