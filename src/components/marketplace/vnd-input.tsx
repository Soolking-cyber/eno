...
const chip = 'rounded-full bg-[#f1f5f9] px-2.5 py-1 text-xs font-semibold text-[#475569] transition-colors hover:bg-[#e8f1fb] hover:text-[#0a66c2] disabled:opacity-40 disabled:hover:bg-[#f1f5f9] disabled:hover:text-[#475569] cursor-pointer'

/**
 * VND amount input. VND has many zeros (12.000.000), so typing is made fast:
 * live dot-grouping as you type, a "+000" append, ×1.000 / ×1.000.000 unit
 * multipliers (type "12" → tap "tr" → 12.000.000), optional preset chips, and a
...
export function VndInput({
  ...
  return (
    <div className={className}>
      ...
      <p className={cn('mt-1 h-4 text-xs font-semibold', n > 0 ? 'text-[#0a66c2]' : 'text-transparent')}>
        = {vndWords(n, lang) || '—'}
      </p>
      ...
      <div className="mt-1.5 flex flex-wrap gap-1.5">
        ...
        {digits ? <button type="button" onClick={() => set('')} className={cn(chip, 'text-[#94a3b8]')}>{tr('Clear', 'Xóa')}</button> : null}
      </div>
    </div>
  )
}
