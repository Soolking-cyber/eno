...
/**
 * ⛔ THE MARKS BELOW ARE THE OFFICIAL SINGLE PATHS FROM `simple-icons` (CC0), COPIED IN RATHER THAN
 * DRAWN BY HAND — a hand-traced brand mark is wrong in a way nobody reviews. Same mono pattern as
...
export function Footer() {
   ...
   * regex, so the interpolated label this replaced was invisible to it — the nine machine-translated
   * languages would have read an English aria-label forever (a reviewer's catch). Keyed by channel so
   * the list in src/lib/socials.ts stays the single source of what we link to.
   */
  const socialAria: Record<Social['key'], string> = {
    facebook: tr('ENO on Facebook', 'ENO trên Facebook'),
    'facebook-group': tr('ENO on the Facebook group', 'ENO trên nhóm Facebook'),
    instagram: tr('ENO on Instagram', 'ENO trên Instagram'),
          ...
          <div className="col-span-2 space-y-3">
            ...
            <div className="-mx-2 flex flex-wrap items-center gap-1 pt-1">
              {/* ⛔ ONE LIST, NOT SEVEN HAND-WRITTEN ANCHORS (src/lib/socials.ts). The row carried three
                  ...
                  rel or a missing hover class. A follower count renders only where socials.ts has a
              ...
              {SOCIALS.map((s) => {
                ...
                return (
                  <a
                    ...
                    target="_blank"
                    /* `me` only where the channel IS eno — see the flag's note in socials.ts. */
                    rel={s.me ? 'noopener noreferrer me' : 'noopener noreferrer'}
                    aria-label={socialAria[s.key]}
                  ...
                  >
                    ...
                    {/* ⚠️ A VISIBLE TAG, BECAUSE THE PAGE AND THE GROUP WEAR THE SAME FACEBOOK MARK
                        and a tooltip does not exist on touch (a reviewer's catch, twice). Two
                        alternatives were ruled out: a second path in the brand mark breaks the
                        single-path brand-glyph policy at the top of this file, and the sprite's
                        `Users` glyph is NOT in CRITICAL_GLYPHS — drawing it here would re-bind
                        `glyphs-rest.svg` (~188 KB) to every page, which is the regression the same
                        commit removes (src/lib/critical-glyphs.guard.test.ts). */}
                    {s.key === 'facebook-group' && (
                      <span className="text-2xs font-semibold">{tr('Group', 'Nhóm')}</span>
                    )}
                    {/* `> 0`, not `!= null`: a channel that genuinely has zero followers should show
                        no badge rather than a literal "0" (a reviewer's catch). */}
                    {s.followers != null && s.followers > 0 && (
                      <span className="text-2xs font-semibold tabular-nums" title={`${name}: ${s.followers.toLocaleString(lang === 'vi' ? 'vi-VN' : 'en-US')}`}>
