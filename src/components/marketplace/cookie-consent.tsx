...
export function CookieConsent() {
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-sm rounded-3xl bg-card p-6 shadow-overlay animate-in fade-in zoom-in-95 duration-150">
        <Mascot name="cookie" className="mx-auto h-28 w-28 text-foreground" />

        {view === 'ask' ? (
          <>
            <h2 className="mt-3 text-center text-lg font-bold text-foreground">{tr('Want results made for you?', 'Muốn kết quả dành riêng cho bạn?')}</h2>
            <p className="mt-1.5 text-center text-sm leading-relaxed text-body">
              {tr(
          ...
          <>
            <h2 className="mt-3 text-center text-lg font-bold text-foreground">{tr('Your choices', 'Lựa chọn của bạn')}</h2>
