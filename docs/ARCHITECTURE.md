...
- ⛔ `experimental.inlineCss: **false**` (grep `inlineCss` in `next.config.ts` — ~`:296` as of 2026-08-23, with the reasoning in the comment directly above it; docs-lint does not verify line numbers, so trust the symbol, not the `:NN`) — **this doc previously said `true` at `:13` and "Do not regress this"; that was stale and inverted.** Inlining WAS enabled to kill a ~570 ms render-blocking stylesheet round-trip, but the A/B in `docs/perf-phase1.md` (perf Phase 1, 2026-07-19) measured what it actually costs under RSC: the flight payload carries the stylesheet descriptor twice more, so the CSS ships ~3× and the homepage HTML measured **876 KB decoded / 126 KB gzip inlined vs 314 KB / 35 KB with a normal cacheable `<link>`** — ~90 KB gzip added to EVERY page's HTML, re-downloaded on every navigation. Turning it back off cost ~+0.3–0.4 s FCP on cold Slow-4G and left LCP unchanged (production, revision eno-vn-00036: LCP 1572 ms, CLS 0.0023, HTML 354 KB / 23.6 KB brotli). **Re-enabling it is a regression, not a restoration** — read `docs/perf-phase1.md` (§"What changed" B, and the FCP trade-off note) before touching it.
